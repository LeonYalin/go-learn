	...
	for _, test := range tests {
		if test.method == "GET" {
			...
			if resp.StatusCode != test.expectedStatusCode {
				t.Errorf("for %s expected %d, but got %d", test.name, resp.StatusCode, test.expectedStatusCode)
			}
		}
	}
}
...
func TestRepository_PostReservation(t *testing.T) {
	...
	reqBody = "start_date=2050-01-01"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2050-01-02")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=J") // must be 3 characters long
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Smith")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "email=john@smith.com")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "phone=123456789")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "room_id=1")
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler failed then trying to fail inserting reservation: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
